...
export const API_BASE = 'https://cd.210k.cc';
...
export async function postQuery(token, payload) {
...
export async function getTask(token, taskId) {
  const res = await fetch(`${API_BASE}/api/task/${encodeURIComponent(taskId)}`, {
    method: 'GET',
    headers: authHeaders(token),
  });
  return handle(res);
...
export async function batchTaskStatus(token, taskIds) {
...
export async function postDetail(token, payload) {
...
export async function waitForTask(token, taskId, { onProgress, signal, intervalMs = 2000, maxAttempts = 90 } = {}) {
  for (let i = 0; i < maxAttempts; i++) {
    ...
    let state;
    try {
      state = await getTask(token, taskId);
    } catch (err) {
...
// 提交查询并等待结果（单页，便利方法）
export async function runQuery(token, payload, { onProgress, signal } = {}) {
  const submit = await postQuery(token, payload);
  ...
  if (submit.tasks) {
    ...
    if (first?.task_id) {
      ...
      const result = await waitForTask(token, first.task_id, { onProgress, signal });
  ...
  if (submit.task_id) {
    onProgress?.({ status: 'pending', position: submit.position });
    const result = await waitForTask(token, submit.task_id, { onProgress, signal });
